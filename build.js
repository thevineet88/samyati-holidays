...
function loadPackages() {
  const files = fs.readdirSync(PACKAGES_DIR).filter(f => f.endsWith('.json')).sort();
...
function renderPackage(pkg, allPkgs) {
  const catLabels = {
    spiritual: 'Spiritual', adventure: 'Adventure', beach: 'Beach',
    nature: 'Nature & Hills', 'ladies-special': 'Ladies Special',
  ...
  const transportIcon = (pkg.transport === 'Flight') ? '&#9992;' :
                        (pkg.transport && pkg.transport.includes('Bus')) ? '&#128663;' : '&#128642;';

  const itinerary = (pkg.itinerary || []).map(day => {
    const meals = day.meals ? `<span class="text-xs font-poppins font-semibold text-orange">&#127858; ${h(day.meals)}</span>` : '';
...
const travelCards = pkg.travel ? [
  ...
  const waBook = encodeURIComponent('Hi Samyati Holidays! I am interested in ' + pkg.title + '. Please share details and booking process.');
  const waQuestion = encodeURIComponent('Hi Samyati Holidays! I have a question about ' + pkg.title + '. Can you help?');
  const waGeneral = encodeURIComponent('Hi Samyati Holidays! I would like to know more about your tour packages.');

  const related = (allPkgs || []).filter(p => p.slug !== pkg.slug).sort(() => Math.random() - 0.5).slice(0, 3);
  ...
  const vars = {
    ...
    TRANSPORT_BADGE: pkg.transport ? `<span class="bg-white/20 text-white text-sm font-semibold px-4 py-2 rounded-full backdrop-blur-sm">${h(pkg.transport)}</span>` : '',
    OVERVIEW_SECTION: pkg.subtitle ? `<div><p class="section-eyebrow">Package Overview</p><h2 class="section-title text-2xl md:text-3xl mt-1 mb-4">${h(pkg.subtitle)}</h2>${pkg.overview ? `<p class="text-gray-600 leading-relaxed mb-4">${h(pkg.overview)}</p>` : ''}</div>` : '',
    HIGHLIGHTS_SECTION: (pkg.highlights || []).length > 0 ? `<div><div class="section-pin-divider mb-6"><div class="pin-icon"></div><div class="line"></div><span class="section-eyebrow">Trip Highlights</span></div><div class="grid grid-cols-1 md:grid-cols-2 gap-3">${(pkg.highlights || []).map(hl => `<div class="flex items-start gap-3 p-4 bg-light-gray rounded-lg"><span class="text-orange font-bold text-lg mt-0.5">&#9733;</span><span class="text-sm">${h(hl)}</span></div>`).join('\n          ')}</div></div>` : '',
    ITINERARY_ITEMS: itinerary,
    INCLUSION_ITEMS: incItems,
    EXCLUSION_ITEMS: excItems,
    ADVISORY_BLOCK: pkg.advisory ? `<div class="bg-orange/10 border-l-4 border-orange rounded-r-xl p-4"><div class="flex gap-3 items-start"><svg class="w-5 h-5 text-orange flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg><div><p class="font-poppins font-semibold text-navy text-sm mb-1">Important Advisory</p><p class="text-gray-700 text-sm">${h(pkg.advisory)}</p></div></div></div>` : '',
    TRAVEL_SECTION: pkg.travel ? `<div><p class="section-eyebrow">Travel Details</p><h2 class="section-title text-2xl mt-1 mb-6">Travel Details</h2><div class="grid grid-cols-1 md:grid-cols-3 gap-4">${travelCards}</div></div>` : '',
    PAYMENT_SECTION: (pkg.installments || []).length > 0 ? `<section class="mb-10"><div class="section-pin-divider mb-6"><div class="pin-icon"></div><div class="line"></div><span class="section-eyebrow">Payment Schedule</span></div><div class="overflow-x-auto rounded-xl border border-gray-200"><table class="pricing-table"><thead><tr><th>Installment</th><th>Amount</th><th>Due By</th></tr></thead><tbody>${installRows}</tbody></table></div></section>` : '',
    CANCELLATION_SECTION: (pkg.cancellation || []).length > 0 ? `<section class="mb-10"><div class="section-pin-divider mb-6"><div class="pin-icon"></div><div class="line"></div><span class="section-eyebrow">Refund &amp; Cancellation Policy</span></div><div class="overflow-x-auto rounded-xl border border-gray-200"><table class="pricing-table"><thead><tr><th>Cancellation Date</th><th>Amount Deducted</th></tr></thead><tbody>${cancelRows}</tbody></table></div><p class="text-xs text-gray-500 mt-3">* First installment is non-refundable in all cases.</p></section>` : '',
    NOTES_SECTION: (pkg.notes || []).length > 0 ? `<div><div class="section-pin-divider mb-4"><div class="pin-icon"></div><div class="line"></div><span class="section-eyebrow">Points to Note</span></div><div class="bg-gray-50 rounded-xl p-5"><ul class="space-y-2 text-sm text-gray-700">${notesHtml}</ul></div></div>` : '',
    RELATED_SECTION: related.length > 0 ? `<section class="py-12 bg-light-gray"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><h2 class="font-poppins font-bold text-navy text-2xl mb-8">You Might Also Like</h2><div class="grid grid-cols-1 md:grid-cols-3 gap-6">${relatedCards}</div></div></section>` : '',
  };
...
if (validPackages.length === 0) {
  console.error('No valid packages found in', PACKAGES_JSON);
  process.exit(1);
}
...
const rootFiles = ['about.html', 'contact.html', 'index.html', 'netlify.toml', 'packages.html', 'package-detail.html', '_redirects'];
...
const jsExport = '// Auto-generated mirror of packages.json - DO NOT EDIT DIRECTLY\n' +
  '// Edit packages/*.json and run `node build.js` to regenerate\n' +
  'const SAMYATI_PACKAGES = ' + JSON.stringify(allPackages, null, 2) + ';\n\n' +
